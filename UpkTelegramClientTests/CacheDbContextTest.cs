...
namespace UpkTelegramClientTests.BotCacheDataBase
{
    ...
    public class CacheDbContextTest
    {
        ...
        /// </summary>
        private List<CachedGroupWorkDay> GenerateCachedDaysRange(DateTime startDate, int count)
        {
            return Enumerable.Range(0, count)
                .Select(i => new CachedGroupWorkDay() { Date = startDate.AddDays(i) })
                .ToList();
        }
        #endregion

        /// <summary>
        /// вновь созданная БД не должна содержать записей
        /// </summary>
        [TestMethod]
        public void CreateService_EmptyTest()
        {
            using (var context = new CacheDbContext())
            using (var service = new CacheDbService(context)) {
                Assert.AreEqual(false, context.CachedGroups.Any());
                Assert.AreEqual(false, context.CachedGroupWorkDays.Any());
            }
        }
